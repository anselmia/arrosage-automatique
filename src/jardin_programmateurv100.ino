...
void setup()
{
  ...
  // Serial.begin(9600);
  wdt_enable(WDTO_8S); // watchdog 4 seconde reinitialisation
  Wire.begin();
...
void meteo_screen()
{
  ...
  print_on_screen(90, 22, temperature);
  u8g.drawStr(15, 33, "Humidity : ");
  print_on_screen(90, 33, humidity);

  if (millis() >= meteo_millis + 10000)
  {
    temperature = 0;
    humidity = 0;
    meteo_millis = 0;
...
void ev_actualization()
{
  ...
  // Desactivate auto mode
  if (menu.desactivate_auto == true)
  {
    for (byte i = 0; i < 6; i++)
      eeprom.write(mem_autostate + (10 * (i + 1)), 0);

    menu.desactivate_auto = false;
  }

  // reset auto mode
  if (menu.reset_auto == true)
  {
    for (byte i = 0; i < 6; i++)
